...
class BulletSim(Simulation):
    ...
    def remove_body(self, body: str):
        self.physics_client.removeBody(self._bodies_idx[body])
        del self._bodies_idx[body]
    ...
    # OBJECT MANAGER
    def create_scene(self, position: np.ndarray, radius: float) -> None:
        self.create_plane(z_offset=-0.4)
        self.create_table(length=1.3, width=2, height=0.01)
        self.create_cylinder(position, radius)
    ...
    def create_sphere(self, position: np.ndarray, body: str) -> None:
        """Create a sphere."""
        radius = 0.025
        visual_kwargs = {
            "radius": radius,
    ...
    def create_cylinder(self, position: np.ndarray, radius: Optional[float] = 0.008) -> None:
        visual_kwargs = {
            "radius": radius,
            "length": radius * 1.5,
            "specularColor": np.zeros(3),
            "rgbaColor": np.append(np.random.rand(3), 0.95),
        }
        self.create_geometry(
            "target",
    ...
    def draw_box_and_line(self, position, orientation, i=0):
        self.create_sphere(position, f"sphere_pos_{i}")
        self.create_box(
            f"body_ori_{i}",
            half_extents=np.array([0.25, 0.005, 0.005]),
            position=np.array(position),
            rgba_color=np.array([0.15, 0.15, 0.15, 0.6]),
        )
        self.set_base_pose(f"body_ori_{i}", position, np.array(orientation))

    def remove_hemisphere_poses(self, count=32):
        for i in range(count):
            self.remove_body(f"body_ori_{i}")
            self.remove_body(f"sphere_pos_{i}")
